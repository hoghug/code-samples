...
- Since the site is mostly metaobjects, we built a custom search with Meilisearch that does a few things throughout the site
- Rather than looping through the metaobjects in liquid, it was more consistent to just fetch and display them with MS, I think mostly due to the ability to SORT them in MS, whereas Shopify didn't care about created/updated at on those metaobjects
...
- We created a new object type in Meili "brand_comparisons", each one is ai generated from their respective Brand Reviews and then that creates an actual blog post in the Shopify store. And the "brand_comparison" object ends up just having a reference to the blog post. 
